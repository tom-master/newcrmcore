	...
	return {
		initData: function () {
			return new Promise((resolve, reject) => {
				HROS.request.get('/desk/checkunreadnotifycount', { pageIndex: 1, pageSize: 100 }, function (responseText) {
					if (responseText.IsSuccess) {
		...
		initMessagePlugin: function () {
			HROS.notify.initData().then((msgArr) => {
				var notify = MessagePlugin.init({
					elem: ".notifyList",
					noticeData: msgArr,
					noticeUnReadData: msgArr.filter((a) => a.readStatus === 0).length,
					noticeClick: function (obj) {
						HROS.request.post('/desk/readnotify', { notifyId: $(obj).prop('id') }, function (responseText) {
							if (responseText.IsSuccess) {
								HROS.notify.initMessagePlugin()
							} else {
								NewCrm.msgbox.fail(responseText.Message)
				...
				if (!HROS.notify.initOnce) {
					HROS.notify.initOnce = 1
					HROS.notify.initSignalR(notify)
				}
			})
		},
		initSignalR: function (notify) {
			var connection = new signalR.HubConnectionBuilder().withUrl("/hubs?accountId=" + HROS.CONFIG.memberID).configureLogging(signalR.LogLevel.Information).build();
