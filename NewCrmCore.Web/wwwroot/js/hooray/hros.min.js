		...
		show: function () {
			if ($('#lock').length == 0) {
				if (!HROS.base.checkLogin()) {
					...
					var lock = function () {
						$('#desktop').hide();
						var userinfo = $.parseJSON($.cookie('User'));
...
HROS.signalR = (function () {
	return {
		init: function () {
			var connection = new signalR.HubConnectionBuilder().withUrl("/hubs?userId=" + top.HROS.CONFIG.memberID).configureLogging(signalR.LogLevel.Information).build();
			connection.on("Message", (responseText) => {
				...
				if (Notification && Notification.permission !== "granted") {
					...
					var not = new Notification(message.Title, options);
				}
			});
