...
HROS.app = (function () {
    return {
		...
		*/
        init: function () {
            //绑定'应用市场'点击事件
            document.querySelector('div.add').addEventListener('click', e => {
            ...
            document.querySelectorAll('.appbtn:not(.add)')
                .forEach(f => {
                    f.addEventListener('contextmenu', (e) => {
                        ...
                        switch (f.getAttribute('type')) {
                            case 'app':
                            case 'widget':
                                popupmenu = HROS.popupMenu.app(f);
                                break;
                            case 'pwindow':
                            case 'pwidget':
                                popupmenu = HROS.popupMenu.papp(f);
                                break;
                            case 'folder':
                                popupmenu = HROS.popupMenu.folder(f);
                                break;
                            case 'file':
                                popupmenu = HROS.popupMenu.file(f);
                                break;
                        }
                        let l = (window.innerWidth - e.clientX) < popupmenu.width() ? (e.clientX - popupmenu.width()) : e.clientX;
                        let t = (window.innerHeight - e.clientY) < popupmenu.height() ? (e.clientY - popupmenu.height()) : e.clientY;
                        popupmenu.css({
                            left: l,
                            top: t
                        }).show();
            ...
            //应用窗口上的评分、分享按钮事件
            document.querySelectorAll('#star ul li').forEach(e => {
                e.addEventListener('click', () => {
                    let num = e.getAttribute('num');
                    if (!isNaN(num) && /^[1-5]$/.test(num)) {
                        if (HROS.base.checkLogin()) {
                            HROS.request.post('/appmarket/modifystar', { AppId: document.querySelector('#star').getAttribute('realappid'), StarCount: num }, function (responseText) {
		...
		*/
        set: function () {
            ...
            document.querySelectorAll('#dock-bar .dock-applist .appbtn').forEach(f => f.remove());
            //$('#dock-bar .dock-applist .appbtn').remove();
            //$('#dock-bar .dock-applist').append(dock_append);
            document.querySelector('#dock-bar .dock-applist').append(dock_append);
            ...
            for (let j = 1; j <= HROS.CONFIG.deskCount; j++) {
                ...
                if (desk) {
                    desk.forEach(d => {
                        desk_append += appbtnTemp({
                            ...
                            'id': 'd_' + d.memberId,
                            'appid': d.memberId,
                            'realappid': d.appId === 0 ? d.memberId : d.appId,
                            'imgsrc': d.icon,
                ...
                document.querySelectorAll('#desk-' + j + ' .desktop-apps-container .appbtn').forEach(f => f.remove());
                //$('#desk-' + j + ' .desktop-apps-container .appbtn').remove();
                document.querySelector('#desk-' + j + ' .desktop-apps-container').append(desk_append);
            ...
            document.querySelectorAll('body .quick_view_container').forEach(f => HROS.folderView.get(document.querySelector('#d_' + f.getAttribute('appid'))));
            // $('body .quick_view_container').each(function () {
            //     HROS.folderView.get($('#d_' + $(this).attr('appid')));
            // });

            document.querySelectorAll('#desk .folder-window').forEach(f => {
                HROS.window.updateFolder(f.getAttribute('appid'));
            });
            //如果文件夹窗口为显示状态，则进行更新
        ...
        setPos: function (isAnimate) {
            let addIcon = document.querySelector('i.addicon');
            addIcon.width = HROS.CONFIG.appSize;
            addIcon.height = HROS.CONFIG.appSize;
            // $('i.addicon').width(HROS.CONFIG.appSize).height(HROS.CONFIG.appSize);

            let appBtnImg = document.querySelector('#desk .desktop-container .appbtn img ');
            appBtnImg.width = HROS.CONFIG.appSize;
            appBtnImg.height = HROS.CONFIG.appSize;

            // $('#desk .desktop-container .appbtn img ').width(HROS.CONFIG.appSize).height(HROS.CONFIG.appSize);

            let appBtnSpan = document.querySelector('#desk .desktop-container .appbtn span')
            appBtnSpan.width = Number(HROS.CONFIG.appSize) + 10;
            ...
            if ($('#lock').length === 0) {
                if (!HROS.base.checkLogin()) {
                    ...
                    let lock = function () {
                        ...
                        getTimeDateWeek();
                        let lockFunc = setInterval(function () {
                            //检查是否有恶意修改源程序绕过锁屏界面
                            let iswarning = false;
                            if ($('#desktop').is(':visible')) {
                                iswarning = true;
...
HROS.searchbar = (function () {
    return {
		...
		*/
        ...
        get: function () {
            let oldSearchVal = '';
            var searchFunc = setInterval(function () {
