    ...
    return {
		...
		*/
        init: () => {
            ...
            //绑定应用右击事件
            $('body').on('contextmenu', '.appbtn:not(.add)', (e) => {
                HROS.popupMenu.hide();
                let popupmenu;
            ...
            //应用窗口上的评分、分享按钮事件
            $('body').on('click', '#star ul li', () => {
                let num = $(this).attr('num');
                if (!isNaN(num) && /^[1-5]$/.test(num)) {
                    if (HROS.base.checkLogin()) {
                        HROS.request.post('/appmarket/modifystar', { AppId: $('#star').attr('realappid'), StarCount: num }, (responseText) => {
		...
		*/
        updateXY: (i) => {
            if (HROS.CONFIG.appXY.toLowerCase() !== i) {
                if (HROS.base.checkLogin()) {
                    HROS.request.post('/desk/modifyxy', { appXy: i }, (responseText) => {
                        if (responseText.IsSuccess) {
		...
		*/
        updateSize: (i) => {
            if (HROS.CONFIG.appSize !== i) {
                ...
                HROS.deskTop.resize();
                if (HROS.base.checkLogin()) {
                    HROS.request.post('/desk/modifysize', { appSize: i }, (responseText) => {
                        if (responseText.IsSuccess) {
		...
		*/
        updateVertical: (i) => {
            if (HROS.CONFIG.appVerticalSpacing !== i) {
                ...
                HROS.deskTop.resize();
                if (HROS.base.checkLogin()) {
                    HROS.request.post('/desk/modifyverticalspace', { appVertical: i }, (responseText) => {
                        if (responseText.IsSuccess) {
		...
		*/
        updateHorizontal: (i) => {
            if (HROS.CONFIG.appHorizontalSpacing !== i) {
                ...
                HROS.deskTop.resize();
                if (HROS.base.checkLogin()) {
                    HROS.request.post('/desk/modifyhorizontalspace', { appHorizontal: i }, (responseText) => {
                        if (responseText.IsSuccess) {
		...
		*/
        get: () => {
            ...
            HROS.request.get('/desk/getuserdeskmembers', {}, (responseText) => {
                if (responseText.IsSuccess) {
		...
		*/
        set: () => {
            ...
            if (HROS.VAR.dock !== null && HROS.VAR.dock !== '') {
                HROS.VAR.dock.forEach(f => {
                    dock_append += appbtnTemp({
                        'title': f.name,
                        'type': f.type,
                        'id': 'd_' + f.memberId,
                        'appid': f.memberId,
                        'realappid': f.appId === 0 ? f.memberId : f.appId,
                        'imgsrc': f.icon,
                        'appsize': 48
                    });
                });
            ...
            $('body .quick_view_container').each(() => {
                HROS.folderView.get($('#d_' + $(this).attr('appid')));
            });

            //如果文件夹窗口为显示状态，则进行更新
            $('#desk .folder-window').each(() => {
        ...
        setPos: (isAnimate) => {
            ...
            $('#dock-bar .dock-applist .appbtn').each((i) => {
                $(this).css({
		...
		*/
        add: (id, callback) => {
            if (HROS.base.checkLogin()) {
                HROS.request.post('/appmarket/install', { AppId: id, DeskNum: HROS.CONFIG.desk }, (responseText) => {
                    if (responseText.IsSuccess) {
                        callback && callback();
                    } else {
                        NewCrm.msgbox.fail(responseText.Message);
		...
		*/
        remove: (id, callback) => {
            ...
            if (HROS.base.checkLogin()) {
                HROS.request.post('/desk/uninstall', { memberId: parseInt(id) }, (responseText) => {
                    if (responseText.IsSuccess) {
                        done();
		...
		*/
        move: () => {
            //应用码头应用拖动
            $('#dock-bar .dock-applist').on('mousedown', '.appbtn', (e) => {
                e.preventDefault();
                e.stopPropagation();
                                ...
                                if (HROS.base.checkLogin()) {
                                    if (!HROS.app.checkIsMoving()) {
                                        if (HROS.app.dataDockToFolder(id, from, to)) {
                                            HROS.request.post('/desk/membermove', { model: { MoveType: 'dock-folder', MemberId: id, From: from, To: to } }, (responseText) => {
                                                if (responseText.IsSuccess) {
                                                    HROS.VAR.isAppMoving = false;
                                                } else {
                                                    NewCrm.msgbox.fail(responseText.Message);
            ...
            //桌面应用拖动
            $('#desktop .desktop-apps-container').on('mousedown', '.appbtn:not(.add)', (e) => {
                e.preventDefault();
                e.stopPropagation();
                if (e.button === 0 || e.button === 1) {
                    let oldobj = $(this);
                                ...
                                if (HROS.base.checkLogin()) {
                                    if (!HROS.app.checkIsMoving()) {
                                        if (HROS.app.dataDeskToFolder(id, from, to, desk)) {
                                            HROS.request.post('/desk/membermove', { model: { MoveType: 'desk-folder', MemberId: id, From: from, To: to } }, (responseText) => {
                                                if (responseText.IsSuccess) {
                                                    HROS.VAR.isAppMoving = false;
                                                } else {
                                                    NewCrm.msgbox.fail(responseText.Message);
                                ...
                                if (HROS.base.checkLogin()) {
                                    if (!HROS.app.checkIsMoving()) {
                                        if (HROS.app.dataDeskToDock(id, from, to, boa, desk)) {
                                            HROS.request.post('/desk/membermove', { model: { MoveType: 'desk-dock', MemberId: id, From: from, To: to } }, (responseText) => {
                                                if (responseText.IsSuccess) {
                                                    HROS.VAR.isAppMoving = false;
                                                } else {
                                                    NewCrm.msgbox.fail(responseText.Message);
            ...
            //文件夹内应用拖动
            $('body').on('mousedown', '.folder_body .appbtn, .quick_view_container .appbtn', (e) => {
                e.preventDefault();
                e.stopPropagation();
                if (e.button === 0 || e.button === 1) {
                    let oldobj = $(this);
                                ...
                                if (HROS.base.checkLogin()) {
                                    if (!HROS.app.checkIsMoving()) {
                                        if (HROS.app.dataFolderToFolder(id, from, to, fromFolderId)) {
                                            HROS.request.post('/desk/membermove', { model: { MoveType: 'folder-folder', MemberId: id, From: from, To: to } }, (responseText) => {
                                                if (responseText.IsSuccess) {
                                                    HROS.VAR.isAppMoving = false;
                                                } else {
                                                    NewCrm.msgbox.fail(responseText.Message);
                                ...
                                if (HROS.base.checkLogin()) {
                                    if (!HROS.app.checkIsMoving()) {
                                        if (HROS.app.dataFolderToDock(id, from, to, fromFolderId, boa, desk)) {
                                            HROS.request.post('/desk/membermove', { model: { MoveType: 'folder-dock', MemberId: id, From: from, To: to } }, (responseText) => {
                                                if (responseText.IsSuccess) {
                                                    HROS.VAR.isAppMoving = false;
                                                } else {
                                                    NewCrm.msgbox.fail(responseText.Message);
		...
		*/
        moveScrollbar: () => {
			...
			*/
            for (let i = 1; i <= 5; i++) {
                $('#desk-' + i).on('mousewheel', (event, delta) => {
        ...
        dataDockToFolder: (id, from, to) => {
            let rtn = false;

            $(HROS.VAR.dock).each((i) => {
                if (this.memberId === parseInt(id)) {

                    $(HROS.VAR.folder).each((j) => {
                        if (this.memberId === parseInt(to)) {
                            HROS.VAR.folder[j].apps.push(HROS.VAR.dock[i]);
                            HROS.VAR.folder[j].apps = HROS.VAR.folder[j].apps.sortBy((n) => { return n.memberId; }, true);
        ...
        dataDockToDesk: (id, from, to, boa, desk) => {
            let rtn = false;
        ...
        dataDockToOtherdesk: (id, from, todesk) => {
            let rtn = false;
        ...
        dataDeskToFolder: (id, from, to, desk) => {
            let rtn = false;
            desk = eval('HROS.VAR.desk' + desk);
            $(HROS.VAR.folder).each((i) => {
                if (this.memberId === parseInt(to) && desk[from] !== null) {
                    HROS.VAR.folder[i].apps.push(desk[from]);
                    HROS.VAR.folder[i].apps = HROS.VAR.folder[i].apps.sortBy((n) => { return n.memberId; }, true);
                    desk.splice(from, 1);
                    rtn = true;
                    return false;
                }
        ...
        dataDeskToDock: (id, from, to, boa, desk) => {
            let rtn = false;
            desk = eval('HROS.VAR.desk' + desk);
            if (desk[from] !== null) {
                if (to === 0) {
        ...
        dataDeskToDesk: (id, from, to, boa, desk) => {
            let rtn = false;
            desk = eval('HROS.VAR.desk' + desk);
            if (desk[from] !== null) {
                if (to === 0) {
        ...
        dataDeskToOtherdesk: (id, from, to, boa, todesk, fromdesk) => {
            let rtn = false;
        ...
        dataFolderToFolder: (id, from, to, fromFolderId) => {
            ...
            if (flags === 2) {
                ...
                HROS.VAR.folder[toKey].apps = HROS.VAR.folder[toKey].apps.sortBy((n) => { return n.appid; }, true);
        ...
        dataFolderToDock: (id, from, to, fromFolderId, boa, desk) => {
            let rtn = false;
            desk = eval('HROS.VAR.desk' + desk);
            $(HROS.VAR.folder).each((i) => {
                if (this.memberId === parseInt(fromFolderId) && HROS.VAR.folder[i].apps[from] !== null) {
        ...
        dataFolderToDesk: (id, from, to, fromFolderId, boa, desk) => {
            let rtn = false;
            desk = eval('HROS.VAR.desk' + desk);
        ...
        dataFolderToOtherdesk: (id, from, todesk, fromFolderId) => {
            let rtn = false;
            todesk = eval('HROS.VAR.desk' + todesk);
    ...
    return {
		...
		*/
        init: () => {
            ...
            $('#amg_folder_container').on('contextmenu', '.appbtn', () => {
                return false;
            });
            HROS.appmanage.move();
        ...
        setPos: () => {
            ...
            $('#amg_dock_container .appbtn').each((i) => {
                $(this).css({
                    top: 10,
            ...
            for (let j = 0; j < 5; j++) {
                $('#amg_folder_container .folderItem:eq(' + j + ') .folderInner .appbtn').each((i) => {
                    $(this).css({
        ...
        move: () => {
            ...
            $('#amg_dock_container').on('mousedown', '.appbtn', (e) => {
                e.preventDefault();
                e.stopPropagation();
                if (e.button === 0 || e.button === 1) {
                    let oldobj = $(this);
                        ...
                        if (cy <= 80) {
                            ...
                            if (movegrid !== null) {
                                ...
                                if (HROS.base.checkLogin()) {
                                    if (!HROS.app.checkIsMoving()) {
                                        if (HROS.app.dataDockToDesk(id, from, to, boa, desk)) {
                                            HROS.request.post('/desk/membermove', { model: { MoveType: 'dock-desk', MemberId: id, From: from, To: desk } }, (responseText) => {
                                                if (responseText.IsSuccess) {
                                                    HROS.VAR.isAppMoving = false;
                                                } else {
                                                    NewCrm.msgbox.fail(responseText.Message);
            ...
            $('#amg_folder_container').on('mousedown', '.appbtn:not(.add)', (e) => {
                e.preventDefault();
                e.stopPropagation();
                if (e.button === 0 || e.button === 1) {
                    let oldobj = $(this);
                                    ...
                                    if (HROS.base.checkLogin()) {
                                        if (!HROS.app.checkIsMoving()) {
                                            if (HROS.app.dataDeskToOtherdesk(id, from, to, boa, todesk, fromdesk)) {
                                                HROS.request.post('/desk/membermove',
                                                    { model: { MoveType: 'desk-desk', MemberId: id, From: from, To: todesk } }, (responseText) => {
                                                        if (responseText.IsSuccess) {
                                                            HROS.VAR.isAppMoving = false;
                                                        } else {
                                                            NewCrm.msgbox.fail(responseText.Message);
        ...
        getScrollbar: () => {
            $('#amg_folder_container .folderInner').height($(window).height() - 80);
            $('#amg_folder_container .folderItem').each(() => {
        ...
        moveScrollbar: () => {
            ...
            **  鼠标滚轮
            */
            $('#amg_folder_container .folderInner').on('mousewheel', (event, delta) => {
...
HROS.base = (() => {
    return {
		...
		*/
        init: () => {
            //更新当前用户ID
            HROS.CONFIG.memberID = JSON.parse($.cookie('User')).Id;
            //阻止弹出浏览器默认右键菜单
            $('body').on('contextmenu', () => {
                return false;
            });
            HROS.signalR.init();
            //加载皮肤
            HROS.base.getSkin(undefined, (arg) => {
        ...
        logout: () => {
            HROS.signalR.stop(() => {
                HROS.request.get('/desk/logout', {}, (responseText) => { location.reload(); });
            });
        },
        checkLogin: () => {
            return parseInt(HROS.CONFIG.memberID) !== 0 ? true : false;
        },
        getSkin: (objSkin, callback) => {
            if (typeof objSkin === 'undefined') {
                HROS.request.get('/desk/getskin', {}, (responseText) => {
                    if (responseText.IsSuccess) {
...
HROS.deskTop = (() => {
    return {
        init: () => {
            ...
            $('body').on('click', '#desktop', () => {
                HROS.popupMenu.hide();
                HROS.folderView.hide();
                HROS.searchbar.hide();
                HROS.startmenu.hide();
            }).on('contextmenu', '#desktop', (e) => {
                HROS.popupMenu.hide();
                HROS.folderView.hide();
                HROS.searchbar.hide();
                HROS.startmenu.hide();
		...
		*/
        ...
        updateDefaultDesk: (i) => {
            if (HROS.base.checkLogin()) {
                HROS.request.post('/desk/modifydefaultdesk', { deskNum: i }, (responseText) => {
                    if (responseText.IsSuccess) {
                        NewCrm.msgbox.success('默认桌面更新成功');
                        $('.dock-pagination').find('a').each((k, v) => {
                            if (parseInt(i) === k + 1) {
                                $(this).trigger('click');
                            }
                        });
        ...
        modifyWallpaperSource: (value) => {
            if (HROS.base.checkLogin()) {
                HROS.request.post('/desk/modifywallpapersource', { source: value }, (responseText) => {
                    if (responseText.IsSuccess) {
                        HROS.wallpaper.get(() => { HROS.wallpaper.set(); });
                    } else {
                        NewCrm.msgbox.fail(responseText.Message);
                    }
                });
    ...
    return {
		...
		*/
        init: () => {
            ...
            HROS.dock.move();
            let dockShowtopFunc;
            $('#dock-container').on('mouseenter', () => {
                dockShowtopFunc = setTimeout(() => { $('#dock-container').addClass('showtop'); }, 300);
            }).on('mouseleave', () => {
                clearInterval(dockShowtopFunc);
                $(this).removeClass('showtop');
            });
            $('body').on('contextmenu', '#dock-container', (e) => {
                HROS.popupMenu.hide();
                HROS.folderView.hide();
                HROS.searchbar.hide();
                HROS.startmenu.hide();
            ...
            //绑定应用码头上各个按钮的点击事件
            $('#dock-bar .dock-tool-setting').on('mousedown', () => {
                return false;
            }).on('click', () => {
                if (HROS.base.checkLogin()) {
                    HROS.window.createTemp({
            ...
            $('#dock-bar .dock-tool-style').on('mousedown', () => {
                return false;
            }).on('click', () => {
                if (HROS.base.checkLogin()) {
                    HROS.window.createTemp({
            ...
            $('#dock-bar .dock-tool-appmanage').on('mousedown', () => {
                return false;
            }).on('click', () => {
                HROS.appmanage.set();
            });
            $('#dock-bar .dock-tool-search').on('mousedown', () => {
                return false;
            }).on('click', (e) => {
                e.stopPropagation();
                HROS.searchbar.get();
            });
            $('#dock-bar .pagination').on('mousedown', () => {
                return false;
            }).on('click', () => {
                HROS.dock.switchDesk($(this).attr('index'));
            });
            $('#dock-bar .dock-tool-start').on('mousedown', () => {
                return false;
            }).on('click', () => {
                HROS.startmenu.show();
                return false;
        ...
        updatePos: (pos) => {
            if (typeof pos !== 'undefined' && pos !== HROS.CONFIG.dockPos.toLowerCase()) {
                HROS.CONFIG.dockPos = pos.toLowerCase();
                if (pos === 'none') {
                    HROS.app.dataAllDockToDesk(HROS.CONFIG.desk);
                }
                if (HROS.base.checkLogin()) {
                    HROS.request.post('/desk/modifydockposition', { Pos: pos, DeskNum: HROS.CONFIG.desk }, (responseText) => {
                        if (responseText.IsSuccess) {
		...
		*/
        ...
        getPos: (callback) => {
            HROS.request.get('/desk/getdockpos', {}, (responseText) => {
                if (responseText.IsSuccess) {
...
HROS.folderView = (() => {
    return {
        init: () => {
            $('body').on('click', '.quick_view_container', () => {
                HROS.popupMenu.hide();
            }).on('click', '.quick_view_container_open', () => {
        ...
        get: (obj) => {
            setTimeout(() => {
                ...
                let folderViewId = null;
                if (iswindowopen) {
                    folderViewId = '#qv_' + obj.attr('appid');
                } else {
                    HROS.folderView.hide();
                }

                let sc = '';
                $(HROS.VAR.folder).each(() => {
        ...
        moveScrollbar: () => {
			...
			**  手动拖动
			*/
            $('body').on('mousedown', '.quick_view_container .quick_view_container_list .scrollBar', (e) => {
			...
			**  鼠标滚轮
			*/
            $('body').on('mousewheel', '.quick_view_container_list_in', (event, delta) => {
...
HROS.grid = (() => {
    return {
        ...
        getFolderGrid: () => {
            ...
            let folderGrid = [];
            $('.quick_view_container, .folder-window:visible').each(() => {
                folderGrid.push({
...
HROS.lock = (() => {
    return {
        ...
        init: () => {
            ...
            $('body').on('click', '#lock-info', () => {
                return false;
            });
            $('body').on('click', '#lockbtn', () => {
                if ($('#lockpassword').val() !== '') {
                    HROS.request.post('/desk/unlockscreen', { unlockPassword: $('#lockpassword').val() }, (responseText) => {
                        if (responseText.IsSuccess) {
                            HROS.lock.hide();
                        } else {
                            $('#lockpassword').val('').focus();
            ...
            $('body').on('keydown', '#lockpassword', (e) => {
                if (e.keyCode === '13') {
                    $('#lockbtn').click();
                }
            });
        ...
        show: () => {
            if ($('#lock').length === 0) {
                if (!HROS.base.checkLogin()) {
                    ...
                    let lock = () => {
                        $('#desktop').hide();
                        let userinfo = JSON.parse($.cookie('User'));
...
HROS.popupMenu = (() => {
    return {
        init: () => {
            $('.popup-menu').on('contextmenu', () => {
                return false;
            });
            //动态控制多级菜单的显示位置
            $('body').on('mouseenter', '.popup-menu li', () => {
		...
		*/
        app: (obj) => {
                    ...
                    if (!HROS.app.checkIsMoving()) {
                        ...
                        if (obj.parent().hasClass('dock-applist')) {
                            if (HROS.app.dataDockToOtherdesk(id, from, todesk)) {
                                HROS.request.post('/desk/membermove', { model: { MoveType: 'desk-desk', MemberId: id, From: from, To: todesk } }, (responseText) => {
                                    if (responseText.IsSuccess) {
                                        HROS.VAR.isAppMoving = false;
                                    } else {
                                        NewCrm.msgbox.fail(responseText.Message);
                            ...
                            if (HROS.app.dataFolderToOtherdesk(id, from, todesk, fromfolderid)) {
                                HROS.request.post('/desk/membermove', { model: { MoveType: 'folder-desk', MemberId: id, From: from, To: todesk } }, (responseText) => {
                                    if (responseText.IsSuccess) {
                                        HROS.VAR.isAppMoving = false;
                                    } else {
                                        NewCrm.msgbox.fail(responseText.Message);
            ...
            $('.app-menu a[menu="edit"]').off('click').on('click', () => {
                if (HROS.base.checkLogin()) {
            ...
            $('.app-menu a[menu="del"]').off('click').on('click', () => {
                ...
                HROS.app.remove(obj.attr('appid'), () => {
        ...
        papp: (obj) => {
            ...
            $('.papp-menu a[menu="moveto"]').off('click').on('click', () => {
                ...
                if (HROS.base.checkLogin()) {
                    ...
                    if (obj.parent().hasClass('dock-applist')) {
                        if (HROS.app.dataDockToOtherdesk(id, from, todesk)) {
                            HROS.request.post('/desk/membermove', { model: { MoveType: 'dock-otherdesk', MemberId: id, From: from, To: todesk } }, (responseText) => {
                                if (responseText.IsSuccess) {
                                    HROS.VAR.isAppMoving = false;
                                } else {
                                    NewCrm.msgbox.fail(responseText.Message);
                        ...
                        if (HROS.app.dataDeskToOtherdesk(id, from, to, 'a', todesk, fromdesk)) {
                            HROS.request.post('/desk/membermove', { model: { MoveType: 'desk-desk', MemberId: id, Trom: from, To: todesk } }, (responseText) => {
                                if (responseText.IsSuccess) {
                                    HROS.VAR.isAppMoving = false;
                                } else {
                                    NewCrm.msgbox.fail(responseText.Message);
		...
		*/
        folder: (obj) => {
            ...
            if (obj.parent().hasClass('desktop-apps-container')) {
                $('.folder-menu a[menu="moveto"]').each(() => {
                    if ($(this).attr('desk') === HROS.CONFIG.desk) {
                        $(this).addClass('disabled');
                    }
                });
            ...
            $('.folder-menu a[menu="moveto"]').off('click').on('click', () => {
                let id = obj.attr('appid'),
                    from = obj.index(),
                    to = 99999,
                    todesk = $(this).attr('desk'),
            ...
            $('.folder-menu a[menu="rename"]').off('click').on('click', () => {
                if (HROS.base.checkLogin()) {
                    $.dialog({
                        ...
                        ok: () => {
                            if ($('#folderName').val() !== '') {
                                HROS.request.post('/desk/modifyfolderinfo', { name: $('#folderName').val(), icon: $('.folderSelector img').attr('src'), memberId: parseInt(obj.attr('appid')) }, (responseText) => {
                                    if (responseText.IsSuccess) {
                                        HROS.app.get();
                                    }
                                });
		...
		*/
        file: (obj) => {
            ...
            //绑定事件
            $('.file-menu a[menu="download"]').off('click').on('click', () => {
                $('body').append(fileDownloadTemp({
                    appid: obj.attr('appid')
                }));
                $('.popup-menu').hide();
            });
            $('.file-menu a[menu="del"]').off('click').on('click', () => {
                HROS.app.dataDeleteByAppid(obj.attr('appid'));
                HROS.app.remove(obj.attr('appid'), () => {
                    obj.find('img, span').show().animate({
                        opacity: 'toggle',
		...
		*/
        dock: () => {
            ...
            if (!TEMP.popupMenuDock) {
                ...
                //绑定事件
                $('.dock-menu a[menu="dockPos"]').on('click', () => {
		...
		*/
        desk: () => {
            ...
            if (!TEMP.popupMenuDesk) {
                ...
                //绑定事件
                $('.desk-menu a[menu="orderby"]').on('click', () => {
                    HROS.app.updateXY($(this).attr('orderby'));
                    $('.popup-menu').hide();
                });
                $('.desk-menu a[menu="size"]').on('click', () => {
                ...
                $('.desk-menu a[menu="lock"]').on('click', () => {
                    HROS.lock.show();
                    $('.popup-menu').hide();
                });
...
HROS.searchbar = (() => {
    return {
		...
		*/
        init: () => {
            $('#pageletSearchInput').on('click', () => {
                return false;
            });
            $('#search-suggest .resultBox').on('click', 'li', () => {
            ...
            $('#search-suggest .openAppMarket a, #pageletSearchButton').on('click', () => {
                HROS.searchbar.openAppMarket($('#pageletSearchInput').val());
            });
            $('#pageletSearchInput').on('keydown', (e) => {
                if (e.keyCode === '13') {
    ...
    return {
		...
		*/
        init: () => {
            ...
            $('#startmenu-container').on('mousedown', (e) => {
                e.preventDefault();
            });
            $('#startmenu-container .startmenu-nick a, #startmenu-container .startmenu-avatar img').on('click', () => {
                HROS.startmenu.openUser();
            });
            $('#startmenu-container .startmenu-exit a').on('click', () => {
                HROS.base.logout();
            });
            $('#startmenu-container .startmenu-lock').on('click', () => {
                HROS.lock.show();
            });
            $('#startmenu-container .startmenu-feedback').on('click', () => {
                HROS.window.createTemp({
            ...
            $('#startmenu-container .startmenu a').on('click', () => {
		...
		*/
        getAvatar: () => {
            HROS.request.get('/desk/getuserface', {}, (responseText) => {
                if (responseText.IsSuccess) {
...
HROS.wallpaper = (() => {
    return {
		...
		*/
        get: (callback) => {
            HROS.request.get('/desk/getwallpaper', {}, (responseText) => {
                if (responseText.IsSuccess) {
		...
		*/
        update: (wallpaperShowType, wallpaperId) => {
            function done() {
                HROS.wallpaper.get(() => {
                    HROS.wallpaper.set();
                });
            ...
            if (HROS.base.checkLogin()) {
                if (!wallpaperId) {
                    HROS.request.post('/desk/modifydisplaymodel', { wallPaperShowType: wallpaperShowType }, (responseText) => {
                        if (responseText.IsSuccess) {
                    ...
                    HROS.request.post('/desk/modifywallpaper', { wallpaperId: wallpaperId }, (responseText) => {
                        if (responseText.IsSuccess) {
...
HROS.widget = (() => {
    return {
		...
		*/
        ...
        create: (appid, type, realappid) => {
            ...
            if (!iswidgetopen && $('#d_' + appid).attr('opening') !== 1) {
                ...
                HROS.request.get('/desk/createwindow', { id: appid, type: type }, (responseText) => {
                    if (responseText.IsSuccess) {
        ...
        reduction: () => {
            if ($.cookie('widgetState' + HROS.CONFIG.memberID)) {
                let widgetState = JSON.parse($.cookie('widgetState' + HROS.CONFIG.memberID));
                $(widgetState).each(() => {
        ...
        //根据id验证是否存在cookie中
        checkCookie: (appid, type) => {
            let value = $.cookie('widgetState' + HROS.CONFIG.memberID);
            let flag = false;
            if (value) {
                let widgetState = JSON.parse(value);
                $(widgetState).each(() => {
		...
		*/
        ...
        move: () => {
            $('#desk').on('mousedown', '.widget .move', (e) => {
        ...
        handle: () => {
            $('#desk').on('mousedown', '.widget a', (e) => {
                e.preventDefault();
                e.stopPropagation();
            });
            $('#desk').on('click', '.widget .ha-close', (e) => {
                let obj = $(this).parents('.widget');
                HROS.widget.close(obj.attr('appid'));
            }).on('click', '.widget .ha-star', () => {
                let obj = $(this).parents('.widget');
...
HROS.window = (() => {
    return {
        init: () => {
            ...
            HROS.window.resize();
            //绑定窗口遮罩层点击事件
            $('#desk').on('click', '.window-container .window-mask, .window-container .folder_body', () => {
                HROS.window.show2top($(this).parents('.window-container').attr('appid'), true);
            });
            //屏蔽窗口右键
            $('#desk').on('contextmenu', '.window-container', () => {
                return false;
            });
            //绑定文件夹内事件
            $('#desk').on('click', '.folder_body li', () => {
                return false;
            }).on('contextmenu', '.folder_body .appbtn', (e) => {
                $('.popup-menu').hide();
                $('.quick_view_container').remove();
                let popupmenu;
                switch ($(this).attr('type')) {
		...
		*/
        createTemp: (obj) => {
            ...
            //判断窗口是否已打开
            let iswindowopen = false;
            $('#task-content-inner a.task-item').each(() => {
                if ($(this).attr('appid') === appid) {
                    iswindowopen = true;
                    HROS.window.show2top($(this).attr('appid'));
                    return false;
		...
		*/
        create: (realappid, type) => {
            ...
            if (!iswindowopen && $('#d_' + realappid).attr('opening') !== 1) {
                $('#d_' + appid).attr('opening', 1);
                HROS.request.get('/desk/createwindow', { id: realappid, type: type }, (responseText) => {
                    if (responseText.IsSuccess) {
                        let app = responseText.Model;
                        nextDo({
                            type: app.type,
        ...
        show2top: (appid, isAnimate) => {
            ...
            if (isAnimate) {
                ...
                $('#desk .window-container:not(' + windowId + ')').each(() => {
        ...
        updateFolder: (appid) => {
            ...
            if (sc !== null) {
                ...
                $(windowId).find('.folder_body').html('').append(folder_append).on('contextmenu', '.appbtn', (e) => {
                    $('.popup-menu').hide();
                    $('.quick_view_container').remove();
        ...
        handle: () => {
            $('#desk').on('mousedown', '.window-container .title-bar .title-handle a', (e) => {
                e.preventDefault();
                e.stopPropagation();
            });
            $('#desk').on('dblclick', '.window-container .title-bar', (e) => {
            ...
            }).on('click', '.window-container .star', () => {
                let obj = $(this).parents('.window-container');
                let point = parseFloat(obj.data('info').starcount).toFixed(1);
                $.dialog({
                    title: '给“' + obj.data('info').title + '”打分',
            ...
            }).on('click', '.window-container .share', () => {
                let obj = $(this).parents('.window-container');
                $.dialog({
                    title: '分享应用',
                    width: 370,
        ...
        move: () => {
            $('#desk').on('mousedown', '.window-container .title-bar', (e) => {
                let obj = $(this).parents('.window-container');
                if (obj.attr('ismax') === 1) {
                    return false;
                }
        ...
        resize: (obj) => {
            $('#desk').on('mousedown', '.window-container .window-resize', (e) => {
                let obj = $(this).parents('.window-container');
                let resizeobj = $(this);
                let lay;
                let x = e.clientX;
...
HROS.signalR = (() => {
    return {
        init: () => {
            let connection = new signalR.HubConnectionBuilder().withUrl("/hubs?userId=" + HROS.CONFIG.memberID).configureLogging(signalR.LogLevel.None).build();
            ...
            connection.start().catch(err => NewCrm.msgbox.fail('推送服务连接失败'));
            HROS.signalR.instance = connection;
        },
        stop: (callback) => {
            HROS.signalR.instance.stop();
            HROS.signalR.instance.onclose((err) => {
                if (callback) {
                    callback && callback();
                }
            });
...
HROS.request = (() => {
    return {
        ...
        internalRequset: (url, method, parameter, callback) => {
            let cookie = $.cookie('User');
            if (cookie || ['/desk/landing'].includes(url.toLowerCase())) {
                $.ajax({
                    type: method,
                    url: url,
