                ...
                if (HROS.base.checkLogin()) {
                    HROS.request.post('/desk/modifyxy', { appXy: i }, (responseText) => {
                        if (responseText.IsSuccess) {
                            ...
                            HROS.deskTop.resize();
                            NewCrm.success('图标排列方向更新成功');
                        } else {
                            NewCrm.fail('图标排列方向更新失败');
                        }
                    });
                }
            }
            ...
            if (HROS.CONFIG.appSize !== i) {
                ...
                if (HROS.base.checkLogin()) {
                    HROS.request.post('/desk/modifysize', { appSize: i }, (responseText) => {
                        if (responseText.IsSuccess) {
                            NewCrm.success('更新应用显示尺寸成功');
                        } else {
                            NewCrm.fail('更新应用显示尺寸失败');
                        }
                    });
                }
            }
            ...
            if (HROS.CONFIG.appVerticalSpacing !== i) {
                ...
                if (HROS.base.checkLogin()) {
                    HROS.request.post('/desk/modifyverticalspace', { appVertical: i }, (responseText) => {
                        if (responseText.IsSuccess) {
                            NewCrm.success('更新应用垂直间距成功');
                        } else {
                            NewCrm.fail('更新应用垂直间距失败');
                        }
                    });
                }
            }
            ...
            if (HROS.CONFIG.appHorizontalSpacing !== i) {
                ...
                if (HROS.base.checkLogin()) {
                    HROS.request.post('/desk/modifyhorizontalspace', { appHorizontal: i }, (responseText) => {
                        if (responseText.IsSuccess) {
                            NewCrm.success('更新应用水平间距成功');
                        } else {
                            NewCrm.fail('更新应用水平间距失败');
                        }
                    });
                }
            }
...
HROS.base = (() => {
    return {
		...
		*/
        init: () => {
            //更新当前用户ID
            HROS.CONFIG.memberID = JSON.parse(HROS.localStorage.get('User')).Id;
            //阻止弹出浏览器默认右键菜单
            $('body').on('contextmenu', () => {
                return false;
            });
            ...
            if (typeof pos !== 'undefined' && pos !== HROS.CONFIG.dockPos.toLowerCase()) {
                ...
                if (HROS.base.checkLogin()) {
                    HROS.request.post('/desk/modifydockposition', { Pos: pos, DeskNum: HROS.CONFIG.desk }, (responseText) => {
                        if (responseText.IsSuccess) {
                            NewCrm.success('应用码头位置更新成功');
                            //更新码头位置
                            HROS.dock.setPos();
                            //更新桌面应用
                            HROS.app.set();
                        } else {
                            NewCrm.fail('应用码头位置更新失败');
                        }
                    });
                }
            }
        ...
        show: () => {
            if ($('#lock').length === 0) {
                if (!HROS.base.checkLogin()) {
                    ...
                    let lock = () => {
                        $('#desktop').hide();
                        let userinfo = JSON.parse(HROS.localStorage.get('User'));
...
HROS.widget = (() => {
    return {
		...
		*/
        ...
        create: (appid, type, realappid) => {
            function nextDo(options) {
                ...
                if (HROS.widget.checkCookie(appid, type)) {
                    let widgetState = $.parseJSON(HROS.cookie.get('widgetState' + HROS.CONFIG.memberID));
        ...
        //还原上次退出系统时widget的状态
        reduction: () => {
            if (HROS.cookie.get('widgetState' + HROS.CONFIG.memberID)) {
                let widgetState = JSON.parse(HROS.cookie.get('widgetState' + HROS.CONFIG.memberID));
                $(widgetState).each(function () {
        ...
        checkCookie: (appid, type) => {
            let value = HROS.cookie.get('widgetState' + HROS.CONFIG.memberID);
            let flag = false;
            if (value) {
		...
		*/
        addCookie: (appid, type, top, right) => {
            if (type === 'widget' || type === 'pwidget') {
                let coo = HROS.cookie.get('widgetState' + HROS.CONFIG.memberID);
                ...
                HROS.cookie.set('widgetState' + HROS.CONFIG.memberID, JSON.stringify(widgetState), { expires: 95 });
            }
        },
        removeCookie: (appid, type) => {
            if (type === 'widget' || type === 'pwidget') {
                if (HROS.widget.checkCookie(appid, type)) {
                    ...
                    HROS.cookie.set('widgetState' + HROS.CONFIG.memberID, JSON.stringify(widgetState));
                }
            }
        },
        move: () => {
...
HROS.signalR = (() => {
    return {
        init: () => {
            ...
            connection.start().catch(err => NewCrm.fail('推送服务连接失败'));
...
HROS.request = (() => {
    return {
        get: (url, parameter, callback, authToken) => {
            return HROS.request.internalRequset(url, 'GET', parameter, authToken, callback);
        },
        post: (url, parameter, callback, authToken) => {
            if (HROS.request.state === 'posting') {
                NewCrm.fail(HROS.CONFIG.postOnce);
            } else {
                return HROS.request.internalRequset(url, 'POST', parameter, authToken, callback);
            }
        },
        internalRequset: (url, method, parameter, authToken, callback) => {
            ...
            axios.defaults.timeout = method === 'GET' ? (1000 * 5) : (1000 * 10);
            axios.defaults.transformRequest = (a, d) => {
                if (method === 'POST' && HROS.request.state === 'posting') {
                ...
                if (method === 'POST' && a) {
                    ...
                    for (let key in a) {
                        if (a.hasOwnProperty(key)) {
                            params.append(key, a[key]);
                        }
                    }
                    return params;
                }
            }
            axios.defaults.transformResponse = (d) => {
            ...
            let ax;
            if (method === 'GET') {
                ax = axios.get(url, { params: Object.keys(parameter).length ? parameter : null, cancelToken: source.token })
            } else {
                ax = axios.post(url, parameter)
            }

            ax.then((responseText) => {
                ...
                if (thrown.code && thrown.code === 'ECONNABORTED') {
                    NewCrm.warning('请求超时,请重试');
                } else if (ax.isCancel && ax.isCancel(thrown)) {
                    NewCrm.warning('请不要重复提交');
                } else {
                    NewCrm.fail('请求失败,请重试');
                }
                HROS.request.state = 'normal';
            })
