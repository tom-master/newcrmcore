				...
				if (responseText.IsSuccess) {
					...
					if (typeof responseText === 'object') {
						...
						$.each(responseText.Model, function (k, v) {
							...
							for (const key in HROS.VAR) {
								if (HROS.VAR.hasOwnProperty(key)) {
									if ('desk' + k === key) {
										HROS.VAR[key] = apps
										break;
									}
									...
									if (HROS.base.checkLogin()) {
										if (!HROS.app.checkIsMoving()) {
											if (HROS.app.dataDockToDesk(id, from, to, boa, desk)) {
												HROS.request.post('/Desk/MemberMove', { MoveType: 'dock-desk', NemberId: id, From: from, To: desk }, function (responseText) {
													if (responseText.IsSuccess) {
														HROS.VAR.isAppMoving = false;
													} else {
														NewCrm.msgbox.fail(responseText.Message);
			...
			$('.papp-menu a[menu="moveto"]').off('click').on('click', function () {
				...
				if (HROS.base.checkLogin()) {
					...
					if (obj.parent().hasClass('dock-applist')) {
						...
						if (HROS.app.dataDeskToOtherdesk(id, from, to, 'a', todesk, fromdesk)) {
							HROS.request.post('/Desk/MemberMove', { MoveType: 'desk-desk', MemberId: id, Trom: from, To: todesk }, function (responseText) {
								if (responseText.IsSuccess) {
									HROS.VAR.isAppMoving = false;
								} else {
									NewCrm.msgbox.fail(responseText.Message);
			...
			if (!iswidgetopen && $('#d_' + appid).attr('opening') != 1) {
				...
				function nextDo(options) {
					...
					if (HROS.widget.checkCookie(appid, type)) {
						...
						HROS.widget.addCookie(options.appid, options.type.toLowerCase(), 0, 0);
					}
					TEMP.widgetTemp = {
						'title': options.title,
						'width': options.width,
